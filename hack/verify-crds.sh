...
for f in $(find "${SCRIPT_ROOT}" -name "*.yaml" -type f); do
    ...
    if [[ $(${YQ} '.apiVersion' $f) == "apiextensions.k8s.io/v1beta1" ]]; then
        if [[ $(${YQ} '.spec.validation.openAPIV3Schema.properties.metadata.description' $f) != "null" ]]; then
            echo "Error: cannot have a metadata description in $f"
            FAILS=true
        fi

        if [[ $(${YQ} '.spec.preserveUnknownFields' $f) != "false" ]]; then
            echo "Error: pruning not enabled (.spec.preserveUnknownFields != false) in $f"
            FAILS=true
        fi
    fi

    if [[ $(${YQ} '.metadata.annotations["api-approved.openshift.io"]' $f) == "null" ]]; then

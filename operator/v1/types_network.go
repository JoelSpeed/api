...
type GatewayConfig struct {
	...
	// +kubebuilder:validation:XValidation:rule="self.contains('.') ? self.split('/')[0].split(':').size() <= 6 : self.split('/')[0].split(':').size() <= 8",message="a valid IPv6 address must contain at most 8 segments, or at most 6 segments for a dual address"
